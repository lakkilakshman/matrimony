...
const EditProfileForm = ({ onCancel, onSuccess }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        maritalStatus: currentUser?.maritalStatus || currentUser?.marital_status || '',
        height: currentUser?.height || '',
        weight: currentUser?.weight || '',
        complexion: currentUser?.complexion || '',
        religion: currentUser?.religion || '',
        caste: currentUser?.caste || '',
        education: currentUser?.education || currentUser?.education_level || currentUser?.educationLevel || '',
        occupation: currentUser?.occupationCategory || currentUser?.occupation_category || currentUser?.occupation || '',
        company: currentUser?.company || currentUser?.company_name || currentUser?.companyName || '',
        annualIncome: currentUser?.annualIncome || currentUser?.annual_income || currentUser?.income || '',
        fatherOccupation: currentUser?.fatherOccupation || currentUser?.father_occupation || '',
        motherOccupation: currentUser?.motherOccupation || currentUser?.mother_occupation || '',
        brothers: currentUser?.brothers || '0',
        sisters: currentUser?.sisters || '0',
        permanentAddress: currentUser?.permanentAddress || currentUser?.permanent_address || '',
        raasi: currentUser?.raasi || '',
        star: currentUser?.star || '',
        birthTime: currentUser?.birthTime || currentUser?.birth_time || '',
    ...
    const handleSave = async () => {
        console.log('=== EDIT PROFILE FORM SAVE STARTED ===');

        // Validation for required fields to prevent database crashes
        ...
        console.log('=== EDIT PROFILE FORM SAVE COMPLETED ===');
    };
                ...
                <div className="flex space-x-4">
                    {onCancel && (
                        <Button
                            onClick={onCancel}
                            variant="outline"
                            className="flex-1 w-full border-gold text-maroon hover:bg-gold/10"
                        >
                            Cancel
                        </Button>
                    )}
